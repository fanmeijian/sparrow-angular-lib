...
import { GroupService, JoblevelService } from "@sparrowmini/org-api";
import { switchMap } from "rxjs";
import { OrgCreateComponent } from "../../org/org-create/org-create.component";

@Component({
  selector: "lib-orggroup-create",
  templateUrl: "./orggroup-create.component.html",
  styleUrls: ["./orggroup-create.component.css"],
})
export class OrggroupCreateComponent implements OnInit {
  parentId: any[] = [];
  submit() {
    ...
    if (this.formGroup.valid) {
      ...
      if (this.formGroup.value.id) {
        ...
        this.groupService
          .newOrgGroup(this.formGroup.value,this.parentId.map((a) => a.id))
          .subscribe(() => {
            this.dialogRef.close();
            this.snack.open("保存成功！", "关闭");
          });
