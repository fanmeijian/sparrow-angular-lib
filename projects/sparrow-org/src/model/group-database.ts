...
import { GroupService, JoblevelService, OrganizationService, RoleService } from "@sparrowmini/org-api";
...
export class GroupDynamicDatabase implements DynamicDatabase {
  constructor(
  ...
  getChildren(node: any): Observable<any[]> | undefined {
    const $roles = this.orgService.orgChildren(node.id, "GROUP").pipe(
      map((res) => res.content!),
      switchMap((roles: any[]) =>
        roles.length===0?of([]):
    ...
    return combineLatest($orgs,$roles).pipe(
      map(m=>m[0].concat(m[1]))
    )
  }
